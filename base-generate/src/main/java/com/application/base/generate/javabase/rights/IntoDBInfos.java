...
public class IntoDBInfos {
	...
	private static boolean addSystemSource(List<SystemSource> SystemSources) {
		try {
			Connection connection = MysqlConnection.newConnection();
			Statement stmt = connection.createStatement();
			for (SystemSource SystemSource : SystemSources) {
				...
				sql.append("'"+SystemSource.getSourceName()+"',");
				sql.append(SystemSource.getParentId()+",");
				sql.append("'"+SystemSource.getSourceKey()+"',");
				sql.append("'"+SystemSource.getSourceType()+"',");
				sql.append("'"+SystemSource.getSourceUrl()+"',");
				sql.append(SystemSource.getSourceLevel()+",");
				sql.append("'"+SystemSource.getDescription()+"'");
				sql.append(");");
